...
describe("GET /general/user/:id", () => {
  it("returns the user without password", async () => {
    ...
    const res = await request(makeApp()).get(`/general/user/${u._id}`).set("Authorization", `Bearer ${token}`);
  ...
  it("404s when the user does not exist", async () => {
    const res = await request(makeApp()).get(`/general/user/${new mongoose.Types.ObjectId()}`).set("Authorization", `Bearer ${token}`);
